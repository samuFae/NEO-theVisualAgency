...
import { AsteroidWrapper, OuterRadius, InnerRadius, AsteroidCard, AsteroidCardContent, AsteroidTime, AsteroidDistance, CardUnit } from "./Asteroid.styles"
...
const Asteroid: FC<IAsteroid> = ({ isDangerous, innerSize, outerSize, data, isMobile, isLegend }) => {
    ...
    return (
        <AsteroidWrapper onMouseOver={handleHover} onMouseOut={handleHover}>
            {!!data && <AsteroidCard outerSize={outerSize} visible={isMobile ? true : visible} id={data?.id}>
                <AsteroidCardContent>
                    <h3>
                        {data?.name.replace(/([()])/g, "")}
                    </h3>
