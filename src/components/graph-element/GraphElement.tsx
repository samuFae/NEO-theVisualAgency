...
const GraphElement: FC<IGraphElement> = ({ data, isMobile }) => {
    ...
    const calculateDimensions = useCallback((breakPoints: number[]) => {
        switch (true) {
            ...
            case data.diameterM.min < 1000:
                ...
                setAsteroidInnerSize((data.diameterM.min / 1000) * breakPoints[0] + (data.diameterM.min - breakPoints[0]) / 1000 * breakPoints[1]);
                break;
            default:
                let comparePrevious = ((data.diameterM.min / 10000) * breakPoints[1])
                setAsteroidInnerSize(comparePrevious > 1 ?
                    (1 / comparePrevious * breakPoints[1]) :
                    (data.diameterM.min / 10000) * breakPoints[1] + (data.diameterM.min - breakPoints[1]) / 10000 * breakPoints[2]);
        }
        switch (true) {
            ...
            case data.diameterM.max < 1000:
                setAsteroidOuterSize((data.diameterM.max / 1000) * breakPoints[0] + (data.diameterM.max - breakPoints[0]) / 1000 * breakPoints[1]);
                break;
            default:
                ...
                setAsteroidOuterSize(comparePrevious > 1 ?
                    (1 / comparePrevious * breakPoints[1]) :
                    (data.diameterM.max / 10000) * breakPoints[1] + (data.diameterM.max - breakPoints[1]) / 10000 * breakPoints[2]);
        }
    ...
    useEffect(() => {
        if (!isMobile) {
            if (!!cellWidth && !!cellHeight && !!cellOffset && !!maxDistance) {
                ...
                let left = cellOffset + (approachHour + approachMinutes / 60 - (24 - timeRange)) / timeRange * cellWidth * (graphCols)
    ...
    }, [asteroidOuterSize, cellHeight, cellOffset, cellWidth, data.distanceKm, data.lastApproach, graphCols, graphRows, isMobile, maxDistance, timeRange])

    return (<React.Fragment>

        {((positions.bottom !== 0 && positions.left > -1) || isMobile) &&
            <GraphElementWrapper positions={positions}>
                <Asteroid data={data} isDangerous={data.isDangerous} innerSize={isMobile ? Math.round(asteroidInnerSize) : Math.round(asteroidInnerSize)} outerSize={isMobile ? Math.round(asteroidOuterSize) : Math.round(asteroidOuterSize)} isMobile={isMobile} />
            </GraphElementWrapper>}
    </React.Fragment>
