...
const GraphBackground: FC<IGraphBackground> = () => {
  ...
  const { graphRows, graphCols, visibleGraphRows, visibleGraphCols, visibleGraphCells, graph, timeRange } =
    useGraphStore();
  ...
  return (
    <GraphBackgroundWrapper graph={graph}>
      {[...Array(visibleGraphCells)].map((el, i) => {
        ...
        return (
          <SpaceOccupier key={i} index={i} graph={graph} ref={i === 1 ? offsetCell : i === 2 ? cell : null}>
            {/* print hours from now */}
            {(i === visibleGraphCols || (i < visibleGraphCols && i !== 1)) && (
              <DidaMedium>{timeSinceString}</DidaMedium>
            )}
            {/* print KMs breakpoints */}
            {i % visibleGraphCols === 1 && i % visibleGraphCols !== visibleGraphRows && (
              <DidaSmall>{breakPoints[Math.round(i / visibleGraphCols)]}</DidaSmall>
            )}
            {/* print Distance*/}
            {i === visibleGraphCols * graphRows + 1 && <DidaMedium>Distance</DidaMedium>}
          </SpaceOccupier>
        );
      })}
